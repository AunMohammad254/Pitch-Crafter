...
import { MODELS, IMAGE_MODELS } from "../../utils/geminiApi";
...
export const CustomImageModelSelector = ({ selectedModel, onSelect, className = "" }) => {
  const [isOpen, setIsOpen] = useState(false);

  // Close when clicking outside
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (isOpen && !event.target.closest('.custom-image-model-selector')) {
        setIsOpen(false);
      }
    };
    document.addEventListener('mousedown', handleClickOutside);
  ...
  const selected = IMAGE_MODELS[selectedModel] || IMAGE_MODELS['gemini-2.5-flash-tts'];

  return (
    <div className={`relative mb-6 z-25 custom-image-model-selector ${className}`}>
      <label className="text-xs font-bold uppercase tracking-wider text-(--accent-primary) mb-2 block pl-1">
        Select Image Model
      </label>
      <div className="relative">
